...
async function initTasks() {
  ...
  if (!clientId) return; // No setup yet
...
async function renderTasksTab() {
  const clientId = await getClientId();
  if (!clientId) {
    showTasksView('tasks-setup-view');
    wireSetupButtons();
    return;
  }
  if (!taskState.connected) {
    showTasksView('tasks-connect-view');
    wireConnectButtons();
    return;
  }
  showTasksView('tasks-main-view');
  renderListSelect();
  renderTasksList();
  renderActiveTask();
  wireMainButtons();
...
function showTasksView(id) {
  ['tasks-setup-view','tasks-connect-view','tasks-main-view'].forEach(v => {
    const el2 = el(v);
    if (el2) el2.classList.toggle('hidden', v !== id);
  });
}

// ── SETUP WIZARD ──────────────────────────────────────
function wireSetupButtons() {
  const saveBtn = el('btn-save-client-id');
  if (saveBtn && !saveBtn._wired) {
    saveBtn._wired = true;
    saveBtn.addEventListener('click', async () => {
      const input = el('tasks-client-id-input');
      const errEl = el('tasks-setup-error');
      const val   = (input?.value || '').trim();
      if (!val || !val.includes('.apps.googleusercontent.com')) {
        if (errEl) { errEl.textContent = 'Please paste a valid Client ID ending in .apps.googleusercontent.com'; errEl.style.display = ''; }
        return;
      }
  ...
  const link = el('tasks-setup-link');
  if (link && !link._wired) {
    link._wired = true;
...
function wireConnectButtons() {
  ...
  const reset = el('btn-reset-client-id');
  if (reset && !reset._wired) {
    reset._wired = true;
    reset.addEventListener('click', async () => {
      await chrome.storage.local.remove('gtasks_client_id');
      showTasksView('tasks-setup-view');
      wireSetupButtons();
    });
  }
...
async function connectTasks() {
  ...
  try {
    ...
    if (btn) { btn.textContent = 'Connect Google Tasks'; btn.disabled = false; }
    const msg = e.includes?.('ERR_BLOCKED') || e.includes?.('Access blocked')
      ? 'App not verified — add your email as a Test User in Google Cloud Console'
      : e || 'Could not connect. Check your Client ID and try again.';
    showToast('⚠️', 'Connection failed', msg);
  }
}

// ── DISCONNECT ────────────────────────────────────────
async function disconnectTasks() {
  if (!confirm('Disconnect Google Tasks? Your Client ID will be kept.')) return;
...
async function refreshTasksData(showSpinner) {
  ...
  try {
    const listsData = await tasksApiFetch(`${TASKS_BASE}/users/@me/lists?maxResults=20`);
    taskState.lists = listsData.items || [];
    if (!taskState.activeListId && taskState.lists.length) taskState.activeListId = taskState.lists[0].id;
    const tasksData = await tasksApiFetch(`${TASKS_BASE}/lists/${taskState.activeListId}/tasks?showCompleted=false&maxResults=40`);
    taskState.tasks = (tasksData.items || []).filter(t => t.status !== 'completed');
    await saveTaskCache(taskState.lists, taskState.tasks, taskState.activeListId);
    renderListSelect();
    renderTasksList();
  } catch (e) {
    if (showSpinner) showToast('⚠️', 'Tasks error', String(e));
  } finally {
...
function renderListSelect() {
  ...
  taskState.lists.forEach(list => {
    ...
    opt.value = list.id; opt.textContent = list.title;
...
function renderTasksList() {
  ...
  if (!taskState.tasks.length) {
    if (loading) { loading.style.display = 'block'; loading.textContent = '🎉 No open tasks — great job!'; }
    return;
  }
...
function renderFocusTaskBar() {
  ...
  if (!bar) {
    ...
    else document.querySelector('#panel-focus')?.prepend(bar);
  }
...
async function completeActiveTask() {
  ...
  try {
    await tasksApiFetch(
      `${TASKS_BASE}/lists/${taskState.activeTaskListId}/tasks/${taskState.activeTask.id}`,
      { method: 'PATCH', body: JSON.stringify({ status: 'completed', completed: new Date().toISOString() }) }
    );
    const title = taskState.activeTask.title;
    taskState.tasks = taskState.tasks.filter(t => t.id !== taskState.activeTask.id);
    taskState.activeTask = null; taskState.activeTaskListId = null;
    await saveTaskActive(null, null);
    renderActiveTask(); renderFocusTaskBar(); renderTasksList();
    showToast('✅', 'Task completed!', title);
  } catch (e) {
    showToast('⚠️', 'Could not complete', String(e));
  } finally {
...
function wireMainButtons() {
  ...
  if (disc && !disc._wired) { disc._wired = true; disc.addEventListener('click', disconnectTasks); }
  const ref = el('btn-refresh-tasks');
  if (ref && !ref._wired) { ref._wired = true; ref.addEventListener('click', () => refreshTasksData(true)); }
  const sel = el('task-list-select');
  if (sel && !sel._wired) {
    sel._wired = true;
  ...
  if (done && !done._wired) { done._wired = true; done.addEventListener('click', completeActiveTask); }
}

// Expose TASKS_BASE for tasks.js
